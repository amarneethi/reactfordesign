...
export default function DesignTool() {
  ...
  const addElement = useCallback((factory, parentId = null) => {
    ...
    if (parentId) {
      ...
      setRootChildren(prev => [...prev, el.id]);
    }
    setSelected(el.id);
  }, []);
  ...
  const deleteElement = useCallback((id) => {
    ...
    if (el.parent) {
      ...
      setRootChildren(prev => prev.filter(c => c !== id));
    }
  ...
  const duplicateElement = useCallback((id) => {
    ...
    if (el.parent) {
      ...
      setRootChildren(prev => [...prev, newId]);
    }
    setSelected(newId);
  }, [elements]);
  ...
  useEffect(() => {
    ...
    const onUp = () => { setDragging(null); setResizing(null); setIsPanning(false); setPanStart(null); };
    window.addEventListener("mousemove", onMove);
    window.addEventListener("mouseup", onUp);
    return () => { window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
  }, [dragging, resizing, isPanning, panStart, zoom, updateEl]);

  // ── Keyboard shortcuts ──
  useEffect(() => {
    const onKey = (e) => {
      if (editingText) return;
      if ((e.key === "Delete" || e.key === "Backspace") && selected) { e.preventDefault(); deleteElement(selected); }
      if (e.key === "d" && (e.metaKey || e.ctrlKey) && selected) { e.preventDefault(); duplicateElement(selected); }
      if (e.key === "Escape") setSelected(null);
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [selected, editingText, deleteElement, duplicateElement]);

  // Canvas pan with middle mouse / space
  const onCanvasMouseDown = (e) => {
    if (e.button === 1 || e.target === canvasRef.current || e.target === artboardRef.current) {
    ...
    if (el.type === "rect") {
      ...
      return (
            ...
            if (dragId && dragId !== id) {
              ...
              if (dragEl) {
                if (dragEl.parent) {
                  ...
                  setRootChildren(prev => prev.filter(c => c !== dragId));
                }
        ...
        >
          ...
          {/* Resize handles */}
          {isSelected && typeof el.width === "number" && (
            <>
    ...
    return ids.map(id => {
      ...
      return (
        <div key={id}>
          ...
          {el.children && el.children.length > 0 && renderTree(el.children, depth + 1)}
        </div>
      );
    });
  };
  ...
  const renderProps = () => {
    if (!sel) return <div style={{ padding: 16, color: "#666", fontSize: 12 }}>Select an element to edit its properties.</div>;
      ...
      <div style={{ overflowY: "auto", flex: 1 }}>
        ...
        {sel.type === "rect" && (
          <>
            <Section title="Size">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                <NumInput label="Width" value={sel.width} onChange={v => updateEl(sel.id, { width: v })} min={10} />
  ...
  return (
    <div style={{ display: "flex", width: "100%", height: "100vh", background: "#0d0d15", color: "#e0e0e0", fontFamily: "'Inter', -apple-system, sans-serif", overflow: "hidden", userSelect: (dragging || resizing) ? "none" : "auto" }}>
      ...
      <div style={sidebarStyle}>
        ...
        <div style={{ flex: 1, overflowY: "auto", paddingTop: 4 }}>
          ...
          {rootChildren.length === 0 ? (
            <div style={{ padding: "20px 12px", textAlign: "center", color: "#444", fontSize: 11 }}>Add elements to begin</div>
      ...
      >
        {/* Grid pattern */}
        <div style={{ position: "absolute", inset: 0, backgroundImage: "radial-gradient(circle, #1a1a2e 1px, transparent 1px)", backgroundSize: `${20 * zoom}px ${20 * zoom}px`, backgroundPosition: `${pan.x}px ${pan.y}px`, pointerEvents: "none", opacity: 0.5 }} />
        ...
        <div style={{ position: "absolute", left: "50%", top: "50%", transform: `translate(-50%, -50%) translate(${pan.x}px, ${pan.y}px) scale(${zoom})`, transformOrigin: "center center" }}>
          <div ref={artboardRef} style={{
            width: 800, minHeight: 600, background: "#1a1a2e", borderRadius: 8, border: "1px solid #2a2a3a",
            boxShadow: "0 20px 60px rgba(0,0,0,0.4)",
            display: "flex", flexDirection: "column", alignItems: "flex-start",
            padding: 24, gap: 12, position: "relative",
          }}
            onClick={(e) => { if (e.target === artboardRef.current) setSelected(null); }}
            onDrop={(e) => {
              e.preventDefault();
              const dragId = e.dataTransfer.getData("element-id");
              if (dragId && elements[dragId]) {
                ...
                if (dragEl.parent) {
                  ...
                  setRootChildren(prev => [...prev, dragId]);
                }
              }
            }}
          ...
          >
            {/* Artboard label */}
            <div style={{ position: "absolute", top: -24, left: 0, fontSize: 11, color: "#555", fontWeight: 500 }}>
              Artboard — 800 × 600
            </div>
            {rootChildren.map(id => renderElement(id))}
          </div>
        </div>
